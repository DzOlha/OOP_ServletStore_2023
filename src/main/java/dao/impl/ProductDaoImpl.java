...
public class ProductDaoImpl implements ProductDao {
    ...
    public void addProduct(Product product) {
        ...
        try (Connection connection = cp.getConnection()) {
            ...
            System.out.println("AFTER PREPARE block");
            System.out.println(product.getName() + " " + product.getDescription() + " " + product.getPrice() + " " + product.getType().getId());
            prepareStatement.setString(1, product.getName());
            prepareStatement.setInt(2, product.getPrice());
            prepareStatement.setString(3, product.getDescription());
            prepareStatement.setInt(4, product.getType().getId());
            System.out.println("AFTER BINDING block");
            if (prepareStatement.executeUpdate() <= 0) {
                log.info("Cannot add product.");
            }
            cp.releaseConnection(connection);
